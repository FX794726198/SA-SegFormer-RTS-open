...
import segmentation_models_pytorch as smp  # 仅用于损失
...
# ========== 使用 FCN (纯 ResNet-50 骨干，仅预训练骨干) ==========
class FCNResNet50(nn.Module):
    def __init__(self, num_classes=1, backbone_pretrained=True):
        super().__init__()
        self.model = fcn_resnet50(
            weights=None,  # 不加载分割头预训练
            weights_backbone=ResNet50_Weights.DEFAULT if backbone_pretrained else None,
...
def main():
    ...
    model  = FCNResNet50(num_classes=1, backbone_pretrained=True).to(device)
