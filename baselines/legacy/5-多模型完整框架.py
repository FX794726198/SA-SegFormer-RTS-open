...
def main():
    ...
    val_tf=A.Compose([A.Resize((256,256)),A.Normalize(),ToTensorV2()])
    ...
    # seg models
    for name,net in [
        ('convnext', smp.Unet('tu-convnext_tiny','imagenet',3,1)),
        ('unet',      smp.Unet('resnet34','imagenet',3,1)),
        ('deeplabv3+',smp.DeepLabV3Plus('mobilenet_v2','imagenet',3,1)),
        ('segformer', smp.SegFormer('MiT-B0','imagenet',3,1)),
        ('swinunet',  smp.Unet('swin_tiny_patch4_window7_224','imagenet',3,1))
    ]:
        models[name]=net.to(device)
        optim[name]=torch.optim.AdamW(net.parameters(),1e-4,weight_decay=1e-5)
        sched[name]=torch.optim.lr_scheduler.CosineAnnealingLR(optim[name],200)
        hist[name]=[]
    # detection model
