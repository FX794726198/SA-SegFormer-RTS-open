# 直接在 Jupyter cell 中运行的完整脚本（使用更轻量的 MobileNetV2 encoder 及最小 batch_size 以适配 GPU）
...
def visualize_samples(model, dataset, device, out_dir, n_samples=4, epoch=0):
    ...
    for i, idx in enumerate(indices):
        ...
        pred = torch.sigmoid(model(img.unsqueeze(0).to(device))).detach().cpu().squeeze().numpy()
    ...
    plt.tight_layout(); plt.savefig(os.path.join(out_dir,f'viz_epoch_{epoch:03d}.png'),dpi=300); plt.close()
...
def main():
    ...
    os.makedirs(ckpt_dir,exist_ok=True); os.makedirs(viz_dir,exist_ok=True); os.makedirs(plots_dir,exist_ok=True)
    ...
    for epoch in range(1,201):
        ...
        if val_loss<best_loss:
            best_loss=val_loss
            torch.save(model.state_dict(),os.path.join(ckpt_dir,f'{model_name}_best_model.pth'))
    ...
    plt.figure(); plt.plot(df.epoch,df.train_loss,label='Train Loss'); plt.plot(df.epoch,df.val_loss,label='Val Loss'); plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend(); plt.tight_layout(); plt.savefig(os.path.join(plots_dir,'loss_curve.png'),dpi=300); plt.close()
    plt.figure();
    for m in ['iou','pixel_acc','precision','recall','f1_score']:
        plt.plot(df.epoch,df[m],label=m)
    plt.xlabel('Epoch'); plt.ylabel('Score'); plt.legend(); plt.tight_layout(); plt.savefig(os.path.join(plots_dir,'metrics_curve.png'),dpi=300); plt.close()
